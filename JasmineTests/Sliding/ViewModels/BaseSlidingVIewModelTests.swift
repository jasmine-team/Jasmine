...
class BaseSlidingViewModelTests: XCTestCase {
    ...
    func testSwapTiles() {
        ...
        let viewModel = BaseSlidingViewModel(time: time, gameData: gameData, tiles: ["a", "b", "c", "d", "e", "f"],
                                          rows: rows, columns: columns)
        let delegate = SlidingGameViewControllerDelegateMock()
        viewModel.delegate = delegate
        viewModel.startGame()
        ...
        XCTAssert(viewModel.slideTile(from: Coordinate(row: 0, col: 0), to: Coordinate(row: 1, col: 1)),
    ...
    func testHasGameWon() {
        ...
        let viewModel = BaseSlidingViewModel(time: 3, gameData: gameData, tiles: ["a"], rows: 1, columns: 1)
